...
function createNewUser($conn, $name, $email, $password, $token, $reference)
{
    $sql = "INSERT INTO users (fullname,email,password,authToken, reference) VALUES(?,?,?,?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$name, $email, $password, $token, $reference]);
    return true;
}
...
function getUserByReferralId($conn, $reference)
{
    $sql = "SELECT * FROM users WHERE reference = :reference";
    $stmt = $conn->prepare($sql);
...
function createReferrals($conn, $referralId, $refereeId)
{
    $sql = "INSERT INTO referrals (referralId,refereeId) VALUES(?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$referralId, $refereeId]);
    return true;
}

function getMyReferrals($conn, $userId)
{
    $sql = "SELECT * FROM referrals WHERE referralId =:userId";
    $stmt = $conn->prepare($sql);
    $stmt->execute(['userId'=>$userId]);
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
...
function fetchAllTransactions($conn, $userId)
{
...
// editUser
function editUser($conn, $name, $phone, $id)
{
   try {
    $sql = "UPDATE users  SET fullname = :name, phone = :phone WHERE id =:userId";
    // echo "Here4";
    $stmt = $conn->prepare($sql);
    $stmt->execute(["name" => $name, "phone" => $phone, "userId" => $id]);
    // echo "Here 2";
    return true;
   } catch (\Throwable $err) {
...
// change password
function updateAccount($conn, $bank, $acct_name, $acc_num, $id)
{
    $sql = 'UPDATE users SET bank_name = :bank, account_name = :acct_name, account_number = :acct_num WHERE id = :id';
    $stmt = $conn->prepare($sql);
    $stmt->execute(["bank" => $bank, "acct_name" => $acct_name, "acct_num" => $acc_num, 'id' => $id]);
    return true;
}
