...
$sql=$con->query("SELECT * FROM auth WHERE status = 'active' ORDER BY id") or die("Error2 : ". mysqli_error($con));
...
?>
       <?php echo number_format($count); ?>   Active  Users</p>
      </h2>
      <p><a href="../admin/?p=dashboard"><i class="fa fa-dashboard"></i> Home</a> &nbsp;&nbsp; > &nbsp;&nbsp; <a class="active">Active users</a></p>
    </div>
    ...
    <section class="content" style="margin-top: 0px; padding-top: 0px; padding-right:40px; ">
      ...
      <div class="row">
        <div class="col-md-10 col-xs-12" style="overflow-x:auto;background-color: #fff; margin-left: 10px; border:solid; border-width: thin; border-color: #ddd;">
          <!-- small box -->
...
<?php if($count > 0 ){ ?>
      <div style="margin: 20px; margin-top: 0px;">
        <form method="get" action="#">
          <input type="hidden" name="p" value="active">
        <input type="text" name="q" value="" placeholder="search by name" style="height: 30px; font-size: 15px; padding: 15px; width: 80%; border:solid; border-color: #cccccc;"> 
       </form>
      </div>
      ...
      <th style="border:solid; border-width: thin; border-color: #eee; color: white; background-color: #0060a0;">Deactivate</th> 
...
?>
<tr><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $i; ?><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $name; ?><td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $email; ?> <td style="border:solid; border-width: thin; border-color: #eee;"><?php echo $status; ?><td style="border:solid; border-width: thin; border-color: #eee;"><input type="hidden" name="getUser"value=<?php echo $id?> /><input type="submit" name="deactivate" onclick="deactivate(<?php echo $id ?>, '<?php echo $name ?>')" class="tableButton btn btn-primary" value="Deactivate" /></td></td></td></td></td></tr>
...
<script type="text/javascript">
    function deactivate(id, name)
      ...
      var r = confirm ("Do you really want to deactivate "+name+"'s account");

      if (r == true) {
        var dataString='id='+id+"&act=deactivate";  
        $.ajax({
          type:"GET",
