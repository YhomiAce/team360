...
function updateReferral($conn, $userId)
{
    $sql = "UPDATE referrals SET hasCollectBonus = 1 WHERE refereeId =:id";
    $stmt = $conn->prepare($sql);
    $stmt->execute(["id" => $userId]);
    return true;
}

function insertTransaction($conn, $userId, $type, $desc, $amount)
{
    $sql = "INSERT into transactions (userId, type, description, amount) VALUES(?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$userId, $type, $desc, $amount]);
    return true;
}

function findAUserById($conn, $id) {
    $sql = "SELECT * FROM users WHERE id =:id";
    $stmt = $conn->prepare($sql);
    $stmt->execute(['id'=>$id]);
    $result = $stmt->fetch();
...
function checkReferral($conn, $id)
{
    $sql = "SELECT * FROM referrals WHERE refereeId =? AND hasCollectBonus = 0";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    return $result;
