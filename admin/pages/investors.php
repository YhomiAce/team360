...
function noOfActiveInvestors($conn) {
  $sql = "SELECT * FROM investment WHERE status = 'invested' ORDER BY id DESC LIMIT 0, 200";
  $stmt = $conn->prepare($sql);
  $stmt->execute();
  $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
  return $result;
}
$rows = noOfActiveInvestors($conn);
?>
  <div class="content-wrapper">
    <section class="content-header">
      <div class="row">
        ...
        <?php echo count($rows) ?> Active Investors
        </h2>
        <p><a href="../admin/?p=dashboard"><i class="fa fa-dashboard"></i> Home</a> &nbsp;&nbsp; > &nbsp;&nbsp; <a href="?p=withdrawals" class="active">Withdrawals</a></p>
      </div>
    <!-- <div class="col-md-4" style="text-align: left; margin-top: 10px;">  <a href="?p=new_product"> <button class="btn btn-primary" style="font-size: 16px; font-weight: 600;">Add New Product</button></a>  </div>-->
     ...
     <!-- Main content -->
    <section class="content" style="margin-top: 0px; padding-top: 0px; padding-right:10px; ">
      <!-- Small boxes (Stat box) -->
      <div class="row">
        <div class="col-md-12 col-xs-12" style="background-color: #fff; margin-left: 10px; margin-right: 10px; border:solid; border-width: thin; border-color: #ddd;">
 ...
 <?php foreach($rows as $key=>$row): ?>
  <tr>
    ...
    <td style="border:solid; border-width: thin; border-color: #eee;"><?php echo (getUserDetails($conn, $_SESSION['user'])['phone']); ?></td>
